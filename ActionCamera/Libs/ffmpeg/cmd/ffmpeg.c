    ...
    if (!next_picture) {
        ...
        case VSYNC_CFR:
            ...
            if (frame_drop_threshold && delta < frame_drop_threshold && ost->frame_number) {
                ...
                nb_frames = (int)lrintf(delta);
                if (delta0 > 1.1)
                    nb0_frames = (int)lrintf(delta0 - 0.6);
            }
            break;
        case VSYNC_VFR:
            if (delta <= -0.6)
...
//     }
    ...
    if (transcode() < 0){
        printf("transcode() ==== err");
        exit_program(1);
        goto fail;//#warning ywc 修改ffmpeg
    }
    ti = getutime() - ti;
    if (do_benchmark) {
    ...
    if ((decode_error_stat[0] + decode_error_stat[1]) * max_error_rate < decode_error_stat[1]) {
        printf("(decode_error_stat[0] + decode_error_stat[1]) * max_error_rate < decode_error_stat[1] ==== err");
        exit_program(69);
        goto fail;
    }
#warning ywc 修改ffmpeg
