...
static int opt_map(void *optctx, const char *opt, const char *arg)
{
    ...
    if ((sync = strchr(map, ','))) {
        *sync = 0;
        sync_file_idx = (int)strtol(sync + 1, &sync, 0);
...
static void add_input_streams(OptionsContext *o, AVFormatContext *ic)
{
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        if (codec_tag) {
            uint32_t tag = (uint32_t)strtol(codec_tag, &next, 0);
            if (*next)
...
loop_end:
    ...
    for (i = 0; i < o->nb_metadata_map; i++) {
        char *p;
        int in_file_index = (int)strtol(o->metadata_map[i].u.str, &p, 0);
...
//        return AVERROR(errno);
    ...
    snprintf(filename, sizeof(filename), "vstats_today.log");
